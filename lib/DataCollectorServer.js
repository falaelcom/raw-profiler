...
class DataCollectorServer extends EventEmitter
{
	...
	//	Parameter: `par.createDataCollector(sourceKey: string, config: object): DataCollector | DataCollectorHttpProxy | { feed(targetStats, hit): void, enabled: boolean, isBucketEnabled(bucketKey): boolean }` -
	...
	constructor(par)
	{
		...
		this.configurationCtimes = { commandFile: 0, configurationFile: 0 };	//	if no runtime config is ever loaded, both values stay `0`; compared to client ctimes (also `0` after the initial `/conf` request) will always equal and no configuration delta will be ever computed.
	...
	{
		let ctimes2;
		if (ctimes) ctimes2 = ctimes;
		else
		{
			switch (key)
			{
				case "enabled":
					ctimes2 = { commandFile: new Date().getTime(), configurationFile: this.configurationCtimes.configurationFile };
					break;
				default:
					ctimes2 = { commandFile: this.configurationCtimes.commandFile, configurationFile: new Date().getTime() };
					break;
			}
		}

		this.configurationCache[key] = { ctimes: ctimes2, source, value, oldValue };
		this.configurationCtimes = ctimes2;
		this.emit("configurationChanged", key, value, oldValue, source, ctimes2);
	}
	...
	//
	//		IMPORTANT: Changes to the following REST endpoints mus be reflected also in `Connection.js::__test()`
	run(par)
	{
		...
		app.post("/feed", (req, res) =>
		{
			...
			return res.json({ cts: __ctimesArray(this.configurationCtimes), deltaConfig });
		});
		...
		app.post("/conf", (req, res) =>
		{
			...
			{
				const deltaConfig = this._buildConfigurationDelta(req.body.cts);				//	lists all settings updated after the times specified in `req.body.cts`
				if (!deltaConfig) return res.json({ cts: __ctimesArray(this.configurationCtimes) });		//	no changes in the data
		...
		app.get("/e", (req, res) =>
		{
			return this.runtimeConfigurator.asyncSmartRefresh(() =>
			{
				this.remoteConfigStore.enabled === true ? res.json({ cts: __ctimesArray(this.configurationCtimes) }) : res.status(204).end();
			});
		});
		...
		function __ctimesArray(ctimes)
		{
			return [ctimes.commandFile, ctimes.configurationFile];
		}
	}
	...
	_buildConfigurationDelta(client_cts)
	{
		...
		for (const key in this.configurationCache)
		{
			...
			const client_ctimes = { commandFile: client_cts[0] || 0, configurationFile: client_cts[1] || 0 };
			//	`client_cts[n] === null` means that this client has not yet been configured remotely;
			...
			//	it's possible for a `client_ctimes` time to be larger than the `ctimes` time for an individual setting because `client_ctimes`
			//		reflects the most recent times when the configuration files were changed, while the `ctimes` of individual settings
			//		reflect the modification time of the configuration file holding the setting when the change of the particular setting was recorded
