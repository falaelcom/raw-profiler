...
class DataCollectorServer extends EventEmitter
{
	...
	//		required; a callback used by the `DataCollectorServer` to instanciate new deltaConfig collector instances; for custom implementations, see `DataCollector` and `DataCollectorHttpProxy` for
	...
	constructor(par)
	{
		...
		this.dataCollectors = {};
		this.remoteConfigurationCache = {};											//	`{ "property.name": { ctimes, source, value, oldValue } }`
		...
		this.configStore = {};											//	`{ "property.name": value }`
	}
	...
	run(par)
	{
		...
		app.post('/feed', (req, res) =>
		{
			...
			//console.log(912, req.body.cts, this.remoteConfigurationCache);
			//console.log(913, req.body.cts, this.configStore);

			const deltaConfig = this._buildConfigurationDelta(req.body.cts);	//	lists all settings updated after the times specified in `req.body.cts`

			//console.log(914, req.body.cts, deltaConfig);

			if (!deltaConfig) return res.status(204).end();						//	no changes

			return res.json({ ctimes: this.configurationCtimes, deltaConfig, currentConfig: this.configStore });
		});
		...
		app.post('/conf', (req, res) =>
		{
			...
			{
				const deltaConfig = this._buildConfigurationDelta(req.body.cts);													//	lists all settings updated after the times specified in `req.body.cts`
				//console.log(922, req.body.cts, this.configurationCtimes, deltaConfig, this.configStore);
				if (!deltaConfig) return res.json({ ctimes: this.configurationCtimes, currentConfig: this.configStore });		//	no changes
		...
		app.get('/e', (req, res) =>
		{
			return this.runtimeConfigurator.asyncSmartRefresh(() =>
			{
				this.configStore.enabled === true ? res.json({ ctimes: this.configurationCtimes }) : res.status(204).end();
			});
		});
	...
	getRemoteConfigurationLines(prefix = null)
	{
		...
		let result = [];
		result.push({ setting: fp + "enabled", type: "runtime", value: this.configStore.enabled !== void 0 ? this.configStore.enabled : "(unset)" });
		result.push({ setting: fp + "uri", type: "runtime", value: this.configStore.uri !== void 0 ? this.configStore.uri : "(unset)" });
		result.push({ setting: fp + "requestTimeoutMs", type: "runtime", value: this.configStore.requestTimeoutMs !== void 0 ? this.configStore.requestTimeoutMs : "(unset)" });
		result.push({ setting: fp + "failureTimeoutMs", type: "runtime", value: this.configStore.failureTimeoutMs !== void 0 ? this.configStore.failureTimeoutMs : "(unset)" });
		for (const key in this.configStore) if (key.indexOf("buckets.") === 0) result.push({ setting: fp + key, type: "runtime", value: this.configStore[key] });
		return result;
	}
	...
	printConfigurationLines()
	{
		...
		{
			sb += "\n[raw-profiler] ---------------------------------\n" + `[raw-profiler] deltaConfig collector ${key}` + "\n[raw-profiler] ---------------------------------";
	...
	runtimeConfiguration_changed(key, value, oldValue, source, ctimes)
	{
		...
		{
			case "enabled":
				this.configStore.enabled = value;
				this.remoteConfigurationCache[key] = { ctimes, source, value, oldValue };
				this.configurationCtimes = ctimes;
				this.onConfigurationChanged(key, this.configStore.enabled, oldValue, source, ctimes);
				return;
			case "proxy.uri":
				this.configStore.uri = value;
				this.remoteConfigurationCache[key] = { ctimes, source, value, oldValue };
				this.configurationCtimes = ctimes;
				this.onConfigurationChanged(key, this.configStore.uri, oldValue, source, ctimes);
				return;
			case "proxy.requestTimeoutMs":
				this.configStore.requestTimeoutMs = value;
				this.remoteConfigurationCache[key] = { ctimes, source, value, oldValue };
				this.configurationCtimes = ctimes;
				this.onConfigurationChanged(key, this.configStore.requestTimeoutMs, oldValue, source, ctimes);
				return;
			case "proxy.failureTimeoutMs":
				this.configStore.failureTimeoutMs = value;
				this.remoteConfigurationCache[key] = { ctimes, source, value, oldValue };
				this.configurationCtimes = ctimes;
				this.onConfigurationChanged(key, this.configStore.failureTimeoutMs, oldValue, source, ctimes);
				return;
			case "logger.verbosity":
				this.configStore["logger.verbosity"] = value;
				this.onConfigurationChanged(key, this.configStore["logger.verbosity"], oldValue, source, ctimes);
				return;
			case "logger.logPath":
				this.configStore["logger.logPath"] = value;
				this.onConfigurationChanged(key, this.configStore["logger.logPath"], oldValue, source, ctimes);
				return;
			case "logger.archivePath":
				this.configStore["logger.archivePath"] = value;
				this.onConfigurationChanged(key, this.configStore["logger.archivePath"], oldValue, source, ctimes);
				return;
			case "logger.maxLogSizeBytes":
				this.configStore["logger.maxLogSizeBytes"] = value;
				this.onConfigurationChanged(key, this.configStore["logger.maxLogSizeBytes"], oldValue, source, ctimes);
				return;
			case "logger.maxArchiveSizeBytes":
				this.configStore["logger.maxArchiveSizeBytes"] = value;
				this.onConfigurationChanged(key, this.configStore["logger.maxArchiveSizeBytes"], oldValue, source, ctimes);
				return;
			case "logger.logRequestArchivingModulo":
				this.configStore["logger.logRequestArchivingModulo"] = value;
				this.onConfigurationChanged(key, this.configStore["logger.logRequestArchivingModulo"], oldValue, source, ctimes);
				return;
			case "sortColumn":	//	data collector
				this.configStore["sortColumn"] = value;
				this.onConfigurationChanged(key, this.configStore["sortColumn"], oldValue, source, ctimes);
				return;
		}
	...
	_ensureDataCollector(sourceKey)
	{
		...
		result = this.dataCollectors[sourceKey] = this.createDataCollector(sourceKey, this.configStore);
		this.onInfo(`Profiling client connected: "${sourceKey}".` + "\n[raw-profiler] =================================\n" + "[raw-profiler] Effective config\n[raw-profiler] =================================\n" + this.printConfigurationLines());
		return result;
	}

	_buildConfigurationDelta(client_cts)
	{
		const result = {};
		let hasDelta = false;
		for (const key in this.remoteConfigurationCache)
		{
			const { ctimes, source, value, oldValue } = this.remoteConfigurationCache[key];
