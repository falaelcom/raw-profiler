...
class Profiler extends EventEmitter
{
	...
	//	Function: Fires the "inspectHitBegin" event whenever a new `hit` is created.
	//	Parameter: `hit: object` - the result of a `ProfilerTarget.hit()` call.
	//	Remarks: Allows for diagnostic logging and adding custom data to the `hit` object at the beginning of a profiling hit.
	//	Usage:
	//	```
	//		__pf.instance.on("inspectHitBegin", hit => hit.customStats.push({categoryTitle: "DB conn.", psText: void 0, osText: context.currentDbConnectionCount }));
	//		...
	//		const hit = __pfbegin("DB");
	//		hit = __pfend(hit);
	//	```
	onInspectHitBegin(hit)
	{
		this.emit("inspectHitBegin", hit);
	}

	//	Function: Fires the "inspectHitEnd" event after a `hit` has been finished and before it's been fed to a data collector.
	//	Parameter: `hit: object` - the result of a `ProfilerTarget.hit()` call modified by a `ProfilerTarget.finish()` call.
	//	Remarks: Allows for diagnostic logging and adding custom data to the `hit` object at the end of a profiling hit.
	//	Usage:
	//	```
	//		__pf.instance.on("onInspectHitEnd", hit => hit.customStats.push({categoryTitle: "DB conn.", psText: void 0, osText: context.currentDbConnectionCount }));
	//		...
	//		const hit = __pfbegin("DB");
	//		hit = __pfend(hit);
	//	```
	...
	begin(bucketKey, key, text)
	{
		...
		try
		{
			this.openHitsCount++;
			this.hitCount++;
			const hit = this._ensureProfilerTarget(key, bucketKey).hit(text, this.hitCount, this.openHitsCount);
			this.onInspectHitBegin(hit);
			return hit;
		}
		catch (ex)
		{
	...
	//	Function: `discard(hit: object): null` - remove the `hit` from the ON-time and open hits count stats.
	//	Parameter: `hit: object` - required; the result of the corresponding `Profiler.begin` call.
	//	Returns: Always returns `null`. Recommended as a shortcut for releasing the current profiler hit's state, e.g.:
	//	```
	//	    let hit = __pfbegin("bucketKey1", "key1" [, "text"]);
	...
	discard(hit)
	{
		...
		{
			this.onError(3456344757, "Uncaught exception, please report to raw-profiler vendor", ex);
			return null;
		}
	}
