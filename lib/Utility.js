...
class Utility
{
	...
	//	Returns: the input duration in milliseconds as `"Y years MM months DD days HH:mm:ss.lllms"`.
	...
	static fduration(ms)
	{
		...
		[
			...
			{ postfix: ":", designator: "h", value: dateTime.getHours(), padding: 2 },
			{ postfix: ":", designator: "min", value: dateTime.getMinutes(), padding: 2 },
			{ postfix: ".", designator: "s", value: dateTime.getSeconds(), padding: 2 },
		...
		for (let length = parts.length, i = 0; i < length; ++i)
		{
			...
			include = true;
			if (!designator) designator = part.designator || "";
	...
	//	Function: `fdurationNs(ns: integer): string` - Format duration in nanoseconds as `"Y years MM months DD days HH:mm:ss.lllmsNNNNNNns"`.
	//	Parameter: `ns: integer` - the input duration value in nanoseconds.
	//	Returns: the input duration in nanoseconds as `"Y years MM months DD days HH:mm:ss.lll(NNNNNNns"`.
	//	Remarks: Zero values will be ommited from the output, e.g. `"05 months 13:05.314ms8236472ns"`.
	static fdurationNs(ns)
	{
		const now = new Date();
		const timezoneOffsetMs = now.getTimezoneOffset() * 60 * 1000;
		const ms = Math.floor(ns / 1000000);
		const nsfrac = ns % 1000000;
		const dateTime = new Date(ms + timezoneOffsetMs);
		const parts =
		[
			{ postfix: " years ", value: dateTime.getFullYear() - 1970, padding: 4 },
			...
			{ postfix: ".", designator: "s", value: dateTime.getSeconds(), padding: 2 },
			{ postfix: "ms", value: dateTime.getMilliseconds(), padding: 3 },
			{ postfix: "ns", value: nsfrac, padding: 6 },
		];
	...
	//	Function: `async sleep(ms: integer): void` - pauses execution for the given amount of milliseconds (`ms`).
	//	Parameter: `ms: integer` - required; a non-negative integer specifying the amount of milliseconds to wait.
	static async sleep(ms)
	{
		if (!Number.isInteger(ms) || ms < 0) throw new Error(`Argument is null: "ms".`);
		return new Promise(f => setTimeout(f, ms));
	}
	//#endregion

	//#region Utilities: HTTP
	async fetchWithTimeout(uri, options)
	{
