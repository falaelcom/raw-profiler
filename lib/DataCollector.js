...
class DataCollector extends EventEmitter
{
	...
	static formatStats(stats, time, title, machineStats, executionStats, currentBucketKey, currentHitKey, callback)
	{
		try
		{
			const result = {};
			...
			if (machineStats)
			{
				sb.push(DataCollector.formatMachineStats(machineStats));
				sb.push('\n');
