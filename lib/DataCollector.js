...
class DataCollector extends EventEmitter
{
	...
	//	Parameter: `par.runtimeConfiguration: RuntimeConfiguration` - required; `DataCollector` subscribes to this instance and listens for runtime configuration changes.
	//	Parameter: `par.fallbackConfiguration: { sortColumn: string }` - required; `DataCollector` uses the values specified as properties to this object as initial configuration.
	...
	constructor(par)
	{
		...
		this.sortColumn = this.fallbackConfiguration.sortColumn;
	...
	//	Parameter: `bucketKey: string` - the key of the bucket to test.
	//	Runtime configuration field: `"buckets." + RuntimeConfiguration.escapeConfigurationKeySegment(bucketKey) + ".sortColumn"`
	//	Remarks: Defaults to the default sorting column (see constructor).
	getBucketSortColumn(bucketKey)
	{
		this.runtimeConfiguration.asyncSmartRefresh();
		const key = "buckets." + RuntimeConfiguration.escapeConfigurationKeySegment(bucketKey) + ".sortColumn";
	...
	runtimeConfiguration_changed(key, value, oldValue)
	{
		...
		{
			case "sortColumn":
				this.sortColumn = value || this.fallbackConfiguration.sortColumn;
		...
		return async.whilst(
			...
			function execute(next)
			{
				...
				[
					...
					function (buckets, next)
					{
						return item.logger.logBuckets(item.hit.bucketKey, buckets, next);
					}.bind(this)
				], next);
			}.bind(this),
			callback
