...
class DataCollector extends EventEmitter
{
	...
	//	Parameter: `customStats: [{categoryTitle: string, psText: string | void 0, osText: string | void 0 }]`
	static formatStats(stats, time, title, machineStats, executionStats, customStats, currentBucketKey, currentHitKey, internalStats, callback)
	{
		try
		{
			const result = {};
			...
			if (machineStats)
			{
				sb.push(DataCollector.formatMachineStats(machineStats, customStats, internalStats));
				sb.push('\n');
	...
	//	Parameter: `customStats: [{categoryTitle: string, psText: string | void 0, osText: string | void 0 }]`
	static formatMachineStats(machineStats, customStats, internalStats)
	{
		const delimiter = "│ ";
		const rowSize = 150;
		const psColSize = 40;
		...
		if (customStats && customStats.length)
		{
			for (let length = customStats.length, i = 0; i < length; ++i)
			{
				const item = customStats[i];
				const categoryTitle = item.categoryTitle || "(n/a)";
				const psText = item.psText || "";
				const osText = item.osText || "";
				printMetric(categoryTitle, psText, osText);
			}
		}
