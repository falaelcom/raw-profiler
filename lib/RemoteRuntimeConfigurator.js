...
class RemoteRuntimeConfigurator extends AsyncEventEmitter
{
	...
	//	Function: Fires the "configurationChanged" event whenever `RemoteRuntimeConfigurator`'s own configuration has changed.
	//	Parameter: `key: string` - the full property object path in the form `propName1.propName2.propName2...`.
	//	Parameter: `value: any` - the new value of the property.
	...
	async inspectFeedResponse(responseBody, response)
	{
		switch (response.status)
		{
			case 200:	//	configuration changed
				const { cts, deltaConfig = null } = JSON.parse(responseBody);	//	`deltaConfig` might be `void 0` with the `/conf` endpoint and no setting updates since `this.remoteCtimes`
				this.remoteCtimes = { commandFile: cts[0], configurationFile: cts[1] };
				...
				for (const key in deltaConfig)
				{
					...
					if (key === "enabled")
					{
						this.onConfigurationChanged("enabled", item.value, this._enabled, "remote", this.remoteCtimes);
						this._enabled = item.value;
					}
					await this.onChanged(key, item.value, item.oldValue, "remote", this.remoteCtimes);
				}
				this.onRefreshFinished(true, this.remoteCtimes);
				//console.log(999.001, "configuration changed", this.remoteCtimes, deltaConfig);
				break;
	...
	async inspectConfigPollingResponse(response)
	{
		switch (response.status)
		{
			case 200:
				...
				const { cts } = JSON.parse(responseBody);
				//console.log(999.999, ctimes.commandFile, "old", this.remoteCtimes.commandFile);
				this.remoteCtimes.commandFile = cts[0];
				if (!this._enabled)
				{
					this.onConfigurationChanged("enabled", true, false, "remote", this.remoteCtimes);
					this._enabled = true;
					await this.onChanged("enabled", true, false, "remote", this.remoteCtimes);
				}
				break;
			case 204:
