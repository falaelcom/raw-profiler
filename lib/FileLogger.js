	...
	logBuckets(currentBucketKey, verbosityOverride, buckets, callback)
	{
		...
		try
		{
			...
			[
				...
				function (next)
				{
					if (currentBucket && headerBucket[EVerbosity.Full]) return fs.writeFile(snapshotFilePath, headerBucket[EVerbosity.Full], next);
					else return next();
				},
				function (next)
				{
					if (currentBucket && currentBucket[EVerbosity.Full]) return fs.appendFile(snapshotFilePath, "\n" + currentBucket[EVerbosity.Full], next);
					else return next();
				},
				function (next)
				{
				...
				function (next)
				{
					if (currentBucket && currentBucket[verbosityOverride || this.verbosity]) return fs.appendFile(logFilePath, "\n" + currentBucket[verbosityOverride || this.verbosity], next);
					else return next();
				}.bind(this),
				function (next)
				{
