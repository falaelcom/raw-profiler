...
class FileLogger extends EventEmitter
{
	...
	//				logRequestArchivingModulo: uint <= 100
	//			},
	//			sourceKey: string,
	//		}
	//	```
	constructor(par)
	{
		...
		if (par.fallbackConfiguration.logFullPath !== "" && !par.fallbackConfiguration.logFullPath) throw new Error(`Argument is null: "par.fallbackConfiguration.logFullPath".`); 
		if (par.fallbackConfiguration.archiveFullPath !== "" && !par.fallbackConfiguration.archiveFullPath) throw new Error(`Argument is null: "par.fallbackConfiguration.archiveFullPath".`); 
		...
		if (isNaN(par.fallbackConfiguration.logRequestArchivingModulo) || par.fallbackConfiguration.logRequestArchivingModulo < 0 || par.fallbackConfiguration.logRequestArchivingModulo > 100) throw new Error(`Argument is invalid: "par.fallbackConfiguration.logRequestArchivingModulo".`);
		...
		this.logFullPath = path.resolve(HOME_DIRECTORY, this.fallbackConfiguration.logFullPath, this.sourceKey);
		this.archiveFullPath = this.fallbackConfiguration.archiveFullPath;
	...
	runtimeConfiguration_changed(key, value, oldValue)
	{
		switch (key)
		{
			case "verbosity":
				...
				return;
			case "logFullPath":
				const logFullPath = path.resolve(HOME_DIRECTORY, value || this.fallbackConfiguration.logFullPath, this.sourceKey);
				if (logFullPath === this.logFullPath) return;
				try
				{
					...
					this.onConfigurationChanged(key, this.logFullPath, oldValue);
				}
				catch (ex)
				{
					if (ex.code === "ENOENT")
					{
						...
						{
							this.onError(2462891, `Cannot create log path "${logFullPath}", will keep using the old setting "${this.logFullPath || "(default)"}"`, ex2);
							return;
						}
					}
					this.onError(246289, `Cannot access log path "${logFullPath}", will keep using the old setting "${this.logFullPath || "(default)"}"`, ex);
				}
				return;
			case "archiveFullPath":
				const archiveFullPath = path.resolve(HOME_DIRECTORY, value || this.fallbackConfiguration.archiveFullPath);
				if (archiveFullPath === this.archiveFullPath) return;
				try
				{
					...
					this.onConfigurationChanged(key, this.archiveFullPath, oldValue);
				}
				catch (ex)
				{
					if (ex.code === "ENOENT")
					{
						...
						{
							this.onError(2462811, `Cannot create archive path "${archiveFullPath}", will keep using the old setting "${this.archiveFullPath || "(default)"}"`, ex2);
							return;
						}
					}
					this.onError(246281, `Cannot access archive path "${archiveFullPath}", will keep using the old setting "${this.archiveFullPath || "(default)"}"`, ex);
				}
				return;
