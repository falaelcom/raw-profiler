...
class FileLogger extends EventEmitter
{
	...
	logBuckets(currentBucketKey, verbosityOverride, buckets, callback)
	{
		...
		try
		{
			...
			[
				...
				function (next)
				{
					if (headerBucket[verbosityOverride || this.verbosity]) return fs.appendFile(logFilePath, "\n" + headerBucket[verbosityOverride || this.verbosity], next);
					else return next();
				}.bind(this),
				function (next)
				{
					if (currentBucket[verbosityOverride || this.verbosity]) return fs.appendFile(logFilePath, "\n" + currentBucket[verbosityOverride || this.verbosity], next);
					else return next();
				}.bind(this),
				function (next)
				{
	...
	//		{ setting: "fileLogger.logPath", type: "runtime", value: "__pflogs" },
	//		{ setting: "fileLogger.archivePath", type: "runtime", value: "__pfarchive" },
	...
	getConfigurationLines(prefix = null)
	{
		...
		result.push({ setting: fp + "logPath", type: "runtime", value: this.logPath });
		result.push({ setting: fp + "archivePath", type: "runtime", value: this.archivePath });
	...
	runtimeConfiguration_changed(key, value, oldValue)
	{
		switch (key)
		{
			case "logger.verbosity":
				this.verbosity = value || this.runtimeInitial.verbosity;
			...
			case "logger.logPath":
				...
				const logFullPath = __resolvePath("~/", path.join(value || this.runtimeInitial.logPath, this.sourceKey));
			...
			case "logger.archivePath":
				...
				const archiveFullPath = __resolvePath("~/", value || this.runtimeInitial.archivePath);
