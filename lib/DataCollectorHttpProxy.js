...
class DataCollectorHttpProxy extends EventEmitter
{
	...
	//	Function: `log(bucketKey: string, text: string): void` - Transmits the input logging data to the configured logging server in a non-blocking way.
	...
	log(bucketKey, text)
	{
		...
		{
			time: new Date(),
			bucketKey,								//	the logging server will distinguish between *logging-* and profiling data based on the presence or absense of the `hit` property (see `this.feed`)
			text,
			sourceKey: this.sourceKey,
		};
		return this._postLogData(body, "/feed");	//	no await necessary
	}

	//	Function: `feed(targetStats: object, hit: object): void` - Transmits the input profiling data to the configured logging server in a non-blocking way.
	...
	feed(targetStats, hit)
	{
		...
		const body =
		{
			hit,									//	the logging server will distinguish between logging- and *profiling* data based on the presence or absense of the `hit` property (see `this.log`)
			targetStats,
			sourceKey: this.sourceKey,
		};
		return this._postLogData(body, "/feed");	//	no await necessary
	...
	async _postLogData(data, endpoint)
	{
		return setImmediate(async () =>
		{
			...
			try
			{
				this.runtimeConfigurator.inspectFeedBody(data);
				const body = JSON.stringify(data);
				const response = await DataCollectorHttpProxy.fetchWithTimeout(this.uri.replace(/\/+$/, "") + endpoint,
				{
					method: "POST",
				...
				if (this.runtimeConfigurator.useRemoteConfig)
				{
					try
					{
						this.runtimeConfigurator.inspectFeedResponse(text, response).catch(ex => { throw ex });	//	it's alright not to await here; the response inspection does either nothing or triggers configuration updates and forgets
					...
					catch (ex)
					{
						...
						{
							this.failureTime = new Date();
							this.onError(49576329, `${this.failureCounter} feed(s) lost. Failing request took ${durationMs} ms. Response data was ${JSON.stringify(text)}`, ex);
						}
					}
					return;
				}
				if (text || response.status !== 204)
				{
					...
					{
						this.failureTime = new Date();
						this.onError(49576324, `${this.failureCounter} feed(s) lost. Failing request took ${durationMs} ms. Response data was ${JSON.stringify(text)}`, new Error(`Unexpected response error (status code ${response.status}, data length: ${text.length}).`));
					}
				}
				else
				{
			...
			catch (ex)
			{
				...
				else this.onError(49576327, "HTTP post data exception", ex);
			}
		});
	}
